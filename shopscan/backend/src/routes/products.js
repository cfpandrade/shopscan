    ...
    const products = db
      .prepare(
        `SELECT * FROM products
         WHERE name LIKE ? OR brand LIKE ? OR size LIKE ? OR description LIKE ? OR barcode LIKE ?
         LIMIT 20`
      )
...
router.post('/manual', (req, res) => {
  try {
    const { name, brand, size, description, image_url } = req.body;
    ...
    db.prepare(
      `INSERT INTO products (barcode, name, brand, size, description, image_url)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).run(barcode, name.trim(), brand || null, size || null, description || null, image_url || null);
